...
const actor = (req: Request): Actor => ({
  id: s((req as any).user?.id),
  name: s((req as any).user?.name),
});
...
export const createWarden = async (req: Request, res: Response) => {
  try {
    const { name, email, password, hostelId } = req.body as {
      name: string;
      email: string;
      password: string;
      hostelId?: string;
...
export const getWardens = async (req: Request, res: Response) => {
  const wardens = await prisma.admin.findMany({
    where: { role: "warden" },
...
export const editHostel = async (req: Request, res: Response) => {
  try {
    const { name, gender, totalRooms, address } = req.body as {
      name?: string;
      gender?: string;
      totalRooms?: number;
      address?: string;
    ...
    const hostel = await prisma.hostel.update({
      ...
      data: {
        ...
        address,
        ...(gender ? { gender } : {}),
        ...(totalRooms != null ? { totalRooms: n(totalRooms) } : {}),
      },
    });
    res.json(hostel);
  } catch (error: any) {
...
export const batchCreateRooms = async (req: Request, res: Response) => {
  try {
    const { hostelId, prefix, from, to, capacity, allowedGender } =
      req.body as {
        hostelId: string;
        prefix?: string;
        from: string;
    ...
    if (!hostelId || !from || !to || !capacity)
      return res
        .status(400)
        .json({ message: "hostelId, from, to, capacity are required" });
    const rooms: any[] = [];
    for (let i = n(from); i <= n(to); i++) {
      rooms.push({
        hostelId,
        roomNumber: prefix ? `${prefix}${i}` : String(i),
        capacity: n(capacity),
    ...
    audit(actor(req), "BATCH_CREATE_ROOMS", "Hostel", hostelId, {
      count: created.count,
    });
    res
      .status(201)
      .json({
        message: `Created ${created.count} rooms`,
        count: created.count,
      });
  } catch (error: any) {
    handleError(error, req, res);
...
export const updateRoom = async (req: Request, res: Response) => {
  try {
    const { status, allowedGender, capacity } = req.body as {
      status?: string;
    ...
    const room = await prisma.room.update({
      ...
      data: {
        ...(status ? { status } : {}),
        ...(allowedGender !== undefined ? { allowedGender } : {}),
        ...(capacity != null ? { capacity: n(capacity) } : {}),
      },
    });
    res.json(room);
  } catch (error: any) {
...
export const deleteHostelRooms = async (req: Request, res: Response) => {
  try {
    ...
    audit(actor(req), "DELETE_HOSTEL_ROOMS", "Hostel", hostelId, {
      count: deleted.count,
    });
  ...
  fs.createReadStream(filePath)
    ...
    .on("data", (row: any) => {
      ...
      if (!row.roomNumber || !row.capacity) {
        errors.push({
          row: rowIndex,
          error: "Missing required field(s): roomNumber, capacity",
        });
        return;
      }
      rooms.push({
        hostelId,
        roomNumber: s(row.roomNumber),
        capacity: n(row.capacity),
        allowedGender: row.allowedGender || null,
      });
    })
    .on("end", async () => {
      try {
        ...
        res
          .status(201)
          .json({
            message: `Uploaded ${created.count} rooms`,
            count: created.count,
            errors,
          });
      } catch (e: any) {
    ...
    .on("error", (err: any) => {
      fs.unlinkSync(filePath);
      res.status(400).json({ message: "CSV error", error: err.message });
    });
};
  ...
  try {
    ...
    const restriction = await prisma.hostelRestriction.create({
      data: {
        ...
        priorityOnlyUntil: priorityOnlyUntil
          ? new Date(priorityOnlyUntil)
          : null,
        notes: notes ?? null,
      },
    });
...
export const createAllocationWindow = async (req: Request, res: Response) => {
  try {
    ...
    if (new Date(opensAt) >= new Date(closesAt))
      return res
        .status(400)
        .json({ message: "opensAt must be before closesAt" });
...
export const activateWindow = async (req: Request, res: Response) => {
  try {
    ...
    const win = await prisma.allocationWindow.update({
      where: { id: s(req.params.id) },
      data: { isActive },
    });
    audit(
      actor(req),
      isActive ? "ACTIVATE_WINDOW" : "DEACTIVATE_WINDOW",
      "AllocationWindow",
      s(req.params.id),
    );
    res.json(win);
  ...
  try {
    ...
    res.json({
      students: students.map((s) => {
        const assignment =
          s.assignment?.status === "confirmed" ? s.assignment : null;
        return { ...s, assignment };
      }),
      total,
      page,
...
export const updateStudent = async (req: Request, res: Response) => {
  try {
    const { name, email, year, branch, program, phone, priorityTier } =
      req.body as {
        name?: string;
        email?: string;
        year?: number;
    ...
    const student = await prisma.student.update({
      ...
      data: {
        name,
        email,
        year: year ? n(year) : undefined,
        branch,
        program,
        phone,
        priorityTier: priorityTier != null ? n(priorityTier) : undefined,
      },
    });
    res.json(student);
  } catch (error: any) {
  ...
  fs.createReadStream(filePath)
    ...
    .on("data", (row: any) => {
      ...
      if (
        ...
        errors.push({
          row: rowIndex,
          error: `Row ${rowIndex}: Missing required fields`,
        });
        return;
      }
      students.push({
        rollNumber: s(row.rollNumber).trim(),
        name: s(row.name).trim(),
        email: s(row.email).trim(),
        year: n(row.year),
        branch: s(row.branch).trim(),
        program: s(row.program || "btech").trim(),
        gender: row.gender ? s(row.gender).toLowerCase() : null,
        phone: row.phone ? s(row.phone).trim() : null,
        priorityTier: n(row.priorityTier || 0),
        passwordHash: "",
        mustChangePassword: true,
        onboardingDone: false,
      });
      ...
      try {
        const hashed = await Promise.all(
          students.map(async (st) => ({
            ...st,
            passwordHash: await bcrypt.hash(`${st.rollNumber}@iiituna`, 10),
          })),
        );
        let created = 0;
        let skipped = 0;
        for (const st of hashed) {
          try {
            ...
            errors.push({
              row: hashed.indexOf(st) + 1,
              error: `${st.rollNumber}: ${e.code === "P2002" ? "Already exists" : e.message}`,
            });
            skipped++;
          }
        }
...
export const getTeachers = async (req: Request, res: Response) => {
  try {
    ...
    res.json(
      teachers.map((t) => {
        const assignment =
          t.assignment?.status === "confirmed" ? t.assignment : null;
        return { ...t, assignment };
      }),
    );
  } catch (error: any) {
  ...
  try {
    ...
    if (error.code === "P2002")
      return res
        .status(409)
        .json({
          message: "Teacher with this employeeId or email already exists",
        });
    handleError(error, req, res);
  }
};
...
export const manualOverride = async (req: Request, res: Response) => {
  try {
    ...
    if (!roomId || !windowId || (!studentId && !teacherId))
      return res
        .status(400)
        .json({
          message: "roomId, windowId, and studentId or teacherId are required",
        });
    ...
    const occupiedCount = await prisma.roomAssignment.count({
      where: { roomId, status: "confirmed" },
    });
    if (!force && occupiedCount >= room.capacity)
      return res
        .status(409)
        .json({
          message: `Room is at capacity (${room.capacity} seats). Pass force:true to override.`,
          canForce: true,
        });
    if (studentId)
    ...
    const assignment = await prisma.roomAssignment.create({
      data: {
        ...
        adminOverride: true,
        bookedAt: new Date(),
        notes: notes || "Admin manual override",
      },
    });
    audit(actor(req), "MANUAL_OVERRIDE", "RoomAssignment", assignment.id, {
      studentId,
      teacherId,
      roomId,
      ...
      await prisma.notification
        .create({
          data: {
            ...
            title: "Room Assigned by Admin",
            body: `Admin assigned you to Room ${room.roomNumber} in ${room.hostel.name}.`,
            metadata: { roomId, assignmentId: assignment.id },
          },
        })
        .catch(() => {});
    }
...
export const unallocateRoom = async (req: Request, res: Response) => {
  try {
    ...
    audit(actor(req), "UNALLOCATE_ROOM", "RoomAssignment", assignment.id, {
...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      totalStudents,
      totalCapacity: totalRoomsResult._sum.capacity ?? 0,
      occupiedSeats,
      totalHostels,
      totalTeachers,
    });
...
export const exportAllocations = async (req: Request, res: Response) => {
  const allocations = await prisma.roomAssignment.findMany({
    where: { status: "confirmed" },
    include: {
      student: true,
      teacher: true,
  ...
  const rows = allocations.map((a) => ({
    name: a.student?.name ?? a.teacher?.name ?? "N/A",
    id: a.student?.rollNumber ?? a.teacher?.employeeId ?? "N/A",
    type: a.student ? "Student" : "Teacher",
    year: a.student?.year ?? "-",
    branch: a.student?.branch ?? a.teacher?.department ?? "-",
    hostel: a.room.hostel.name,
    room: a.room.roomNumber,
    bookedAt: new Date(a.bookedAt).toLocaleString(),
    adminOverride: a.adminOverride ? "Yes" : "No",
  }));
  const fields = [
    "name",
    "id",
  ...
  res
    ...
    .attachment("allocations_report.csv")
