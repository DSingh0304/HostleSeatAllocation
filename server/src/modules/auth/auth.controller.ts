...
export const studentLogin = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ...tokens, mustChangePassword: student.mustChangePassword, onboardingDone: student.onboardingDone });
  } catch (error: any) {
...
export const adminLogin = async (req: Request, res: Response) => {
  try {
    ...
    const tokens = await generateTokens({ id: admin.id, role: admin.role, name: admin.name, email: admin.email });
    res.json(tokens);
  } catch (error: any) {
    res.status(400).json({ message: error.message });
  }
...
export const completeOnboarding = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.student.update({
      where: { id: userId },
      data: { passwordHash: hash, gender, mustChangePassword: false, onboardingDone: true },
    });
...
export const changePassword = async (req: Request, res: Response) => {
  try {
    ...
    if (role === 'student') {
      const student = await prisma.student.findUnique({ where: { id: userId } });
      if (!student || !(await comparePassword(oldPassword, student.passwordHash))) {
        return res.status(401).json({ message: 'Current password is incorrect' });
      }
      await prisma.student.update({ where: { id: userId }, data: { passwordHash: await hashPassword(newPassword) } });
    } else if (role === 'teacher') {
      const teacher = await prisma.teacher.findUnique({ where: { id: userId } });
      if (!teacher || !(await comparePassword(oldPassword, teacher.passwordHash))) {
        return res.status(401).json({ message: 'Current password is incorrect' });
      }
      await prisma.teacher.update({ where: { id: userId }, data: { passwordHash: await hashPassword(newPassword), mustChangePassword: false } });
    } else {
      const admin = await prisma.admin.findUnique({ where: { id: userId } });
      if (!admin || !(await comparePassword(oldPassword, admin.passwordHash))) {
        return res.status(401).json({ message: 'Current password is incorrect' });
      }
      await prisma.admin.update({ where: { id: userId }, data: { passwordHash: await hashPassword(newPassword) } });
    }
...
export const adminResetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (targetType === 'student') {
      await prisma.student.update({ where: { id: targetId }, data: { passwordHash: hash, mustChangePassword: true } });
    } else if (targetType === 'teacher') {
      await prisma.teacher.update({ where: { id: targetId }, data: { passwordHash: hash, mustChangePassword: true } });
    } else {
...
export const refresh = async (req: Request, res: Response) => {
  try {
    ...
    const tokens = await generateTokens({ id: decoded.userId, role: decoded.role });
    res.json(tokens);
  } catch (error: any) {
    res.status(400).json({ message: error.message });
  }
