          ...
          room: {
            include: {
              ...
              assignments: {
                ...
                include: {
                  student: {
                    select: { name: true, rollNumber: true, id: true },
                  },
                },
              },
            },
...
export const getEligibleHostels = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!student) return res.status(404).json({ message: "Student not found" });
    const activeWindow = await prisma.allocationWindow.findFirst({
      where: { isActive: true },
    });
    if (!activeWindow) return res.json([]);

    // Window-level eligibility: gender, program, year
    const windowAllowsGender =
      activeWindow.gender === "mixed" || activeWindow.gender === student.gender;
    const windowAllowsProgram =
      activeWindow.allowedPrograms.length === 0 ||
      activeWindow.allowedPrograms.includes(student.program);
    const windowAllowsYear =
      activeWindow.allowedYears.length === 0 ||
      activeWindow.allowedYears.includes(student.year);
    ...
    const eligible = hostels.filter((hostel) => {
      ...
      return hostel.restrictions.some((r) => {
        const yearOk =
          r.allowedYears.length === 0 || r.allowedYears.includes(student.year);
        const programOk =
          r.allowedPrograms.length === 0 ||
          r.allowedPrograms.includes(student.program);
    ...
    const genderFilter = student.gender
      ? ({
          OR: [
            ...
            { allowedGender: null as string | null },
          ],
        } as any)
      : {};
...
export const markNotificationRead = async (req: AuthRequest, res: Response) => {
  ...
  const notif = await prisma.notification.findFirst({
    where: { id: s(req.params.id), studentId },
  });
  if (!notif)
...
export const getNotices = async (req: AuthRequest, res: Response) => {
  try {
    const student = await prisma.student.findUnique({
      where: { id: req.user!.id },
      include: { assignment: { include: { room: true } } },
    });
...
export const getPreferences = async (req: AuthRequest, res: Response) => {
  const preference = await prisma.roomPreference.findUnique({
    where: { studentId: req.user!.id },
  });
  res.json(preference);
};
...
export const setPreferences = async (req: AuthRequest, res: Response) => {
  const studentId = req.user!.id;
  const { windowId, seaterPref } = req.body as {
    windowId: string;
    seaterPref?: number;
  };
  const preference = await prisma.roomPreference.upsert({
    ...
    create: {
      studentId,
      windowId,
      seaterPref: seaterPref ? n(seaterPref) : null,
    },
...
export const sendInvite = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { receiverRollNumber, roomId, windowId } = req.body as {
      receiverRollNumber: string;
      roomId: string;
      windowId: string;
    };
    if (!receiverRollNumber || !roomId || !windowId)
      return res
        .status(400)
        .json({
          message: "receiverRollNumber, roomId, and windowId are required",
        });
    ...
    if (!receiver)
      return res
        .status(404)
        .json({ message: "No student found with that roll number" });
    ...
    if (
      !sender?.gender ||
      !receiver?.gender ||
      sender.gender !== receiver.gender
    ) {
      return res
        .status(400)
        .json({
          message: `Gender mismatch. You (${sender?.gender}) can only invite students of the same gender.`,
        });
    }

    const receiverAssigned = await prisma.roomAssignment.findUnique({
      where: { studentId: receiver.id },
    });
    if (receiverAssigned?.status === "confirmed")
      return res
        .status(409)
        .json({ message: `${receiver.name} is already allocated to a room` });

    const existing = await prisma.roommateInvite.findFirst({
      where: { senderId, receiverId: receiver.id, roomId, status: "pending" },
    });
    if (existing)
      return res
        .status(409)
    ...
    const invite = await prisma.roommateInvite.create({
      data: { senderId, receiverId: receiver.id, roomId, windowId, expiresAt },
    });
    ...
    await prisma.notification
      .create({
        data: {
          ...
          title: "Roommate Invite Received",
          body: `${sender!.name} (${sender!.rollNumber}) invited you to share Room ${room.roomNumber} in ${room.hostel.name}. Respond within 30 minutes.`,
          metadata: { inviteId: invite.id, roomId, senderName: sender!.name },
        },
      })
      .catch(() => {});
    ...
    res
      ...
      .json({
        invite,
        message: `Invite sent to ${receiver.name}. They have 30 minutes to accept.`,
      });
  } catch (e: any) {
    handleError(e, req, res);
  }
...
export const getMyInvites = async (req: AuthRequest, res: Response) => {
  ...
  const [sent, received] = await Promise.all([
    prisma.roommateInvite.findMany({
      where: { senderId: studentId },
      include: {
        receiver: { select: { name: true, rollNumber: true } },
        sender: { select: { name: true, rollNumber: true } },
      },
      orderBy: { createdAt: "desc" },
    }),
    prisma.roommateInvite.findMany({
...
export const respondToInvite = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const invite = await prisma.roommateInvite.findFirst({
      where: { id: inviteId, receiverId: studentId, status: "pending" },
    });
    if (!invite)
      return res
        .status(404)
    ...
    if (action === "accept") {
      ...
      if (occupants >= room.capacity)
        return res.status(409).json({ message: "Room is now full" });
...
export const lockRoomForGroup = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!student?.assignment || student.assignment.status !== "confirmed") {
      return res
        .status(403)
        .json({ message: "You do not have a confirmed room allocation" });
    }
    ...
    if (room && room.privateUntil && new Date(room.privateUntil) > new Date()) {
      return res
        .status(400)
        .json({ message: "Room is already locked for your group." });
    }
    ...
    if (Date.now() - bookedAt > 5 * 60 * 1000) {
      return res
        ...
        .json({
          message:
            "The 5-minute group-lock option has expired. Room is now public.",
        });
    }
    ...
    await prisma.room.update({
      ...
      data: {
        isPublic: false,
        privateUntil: new Date(Date.now() + 10 * 60 * 1000),
      }, // extend to 10 mins
    });
